...
export const actions = {
    async actionLogIn(context: MainContext, payload: { username: string; password: string }) {
        try {
            ...
            if (token) {
                ...
                await dispatchGetUserAccount(context);
    ...
    async actionGetUserAccount(context: MainContext) {
        try {
            const response = await api.getMe(context.state.token);
            if (response.data) {
                commitSetUserAccount(context, response.data);
            }
        } catch (error) {
            await dispatchCheckApiError(context, error);
        }
    },
    async actionUpdateUserAccount(context: MainContext, payload) {
        try {
    ...
    async actionUpdateUserProfile(context: MainContext, payload) {
        try {
            const loadingNotification = { content: 'saving', showProgress: true };
            commitAddNotification(context, loadingNotification);
            const profileId = context.state.userAccount.profile[0].id;
            const response = (
                await Promise.all([
                    api.updateProfile(context.state.token, profileId, payload),
                    await new Promise((resolve, reject) => setTimeout(() => resolve(), 500))
                ])
            )[0];
